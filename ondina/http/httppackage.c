...
http_data_input_stream(GInputStream * stream,gsize * length,GCancellable * cancellable,GError ** error)
{
  ...
  while(g_input_stream_read(stream,&byte,1,cancellable,error))
	...
	{
	  byte_read ++;
	  content = g_realloc(content,byte_read + 2);
	  content[byte_read - 1] = byte;
	  content[byte_read] = 0;

	  if(byte_read >= 4)
	    {
	      if(strncmp((content + byte_read - 4),"\r\n\r\n",4) == 0)
		break;
	    }
	}
    }
  ...
  * dis = g_data_input_stream_new(G_INPUT_STREAM(result));
  g_filter_input_stream_set_close_base_stream(G_FILTER_INPUT_STREAM(dis),TRUE);
